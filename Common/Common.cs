...
public class Range<T>(T s, T e)
    where T : INumber<T>
{
    public T Start = s;
    public T End = e;

    public void Merge(Range<T> r)
    {
        if (r.Start < Start)
        {
            Start = r.Start;
    ...
    public bool Overlaps(Range<T> r)
    {
        return (r.Start <= Start && r.End >= Start) || (r.Start <= End && r.End >= End);
    }
...
public class Ranges<T> where T  : INumber<T> 
{
    ...
    public void Join()
    {
        Range<T> activeRange = null;
        for (int i = 0; i < ranges.Count; i++)
        {
            if (activeRange == null)
            {
                activeRange = new Range<T>(ranges[i].Start, ranges[i].End);
            }

            if (i < ranges.Count - 1)
            {
                if (ranges[i + 1].Start == (activeRange.End + T.One))
                {
                    activeRange.End = ranges[i + 1].End;
                }
                else
    ...
    public void Merge(Range<T> range)
    {
        bool found = false;

        foreach (Range<T> r in ranges)
        {
            if (r.Overlaps(range) || range.Overlaps(r))
            {
                range.Merge(r);
        ...
        // keep them in order
        ranges.Sort((Range<T> left, Range<T> right) => left.Start.CompareTo(right.Start));
    }

    public bool HasSingleHole(T min, T max, ref T result)
    {
        if (ranges.Count == 0)
        {
            return false;
        ...
        // make sure the gap between end of range 1 and start of range 2 is 1...
        if (ranges[0].End + T.One == ranges[1].Start - T.One)
        {
            result = ranges[0].End + T.One;
            return true;
        }
